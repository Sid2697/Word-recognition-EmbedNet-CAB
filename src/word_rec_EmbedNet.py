"""
This file has the code for calculating word accuracy using
...
parser.add_argument(
    ...
    help='path to the topk text embeds'
    )
parser.add_argument(
    '--predictions_file',
    default='gen_files/top_preds_embeds_100_with_conf.txt',
    help='path to the top preds text file options: \
        [top_preds_embeds_with_confidence_1500, top_preds_embeds_all_w\
            ith_confidence, top_preds_embeds_all_with_confidence_tel\
                ugu_deep]'
                )
parser.add_argument(
    '--image_file',
    default='gen_files/image_embed_top_k_100.txt',
    help='path to the text file used for producing image\
        embeddings options: [image_embed_top_k_1500,\
            image_embed_top_k_all, test_ann_1000_pages_Telugu_deep]'
            )
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help='If True we will use confidence score for re-ranking'
    )
parser.add_argument(
    '--cab',
    default=False,
    action="store_true",
    help='If True we will use CAB for improving the word\
...
parser.add_argument(
    ...
    default=33,
    type=int,
    help='Hyperparameter alpha defined for the CAB module'
    )
parser.add_argument(
    '--cab_beta',
    default=1,
    type=int,
    help='Hyperparameter beta defined for the CAB module'
    )
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help='Wheater to use or not use NN'
    )
parser.add_argument(
    '--in_features',
    default=2048,
    type=int,
    help='Size of the input to the neural network'
    )
parser.add_argument(
    '--out_features',
    default=128, type=int,
    help='Size of the output of the neural network'
    )
parser.add_argument(
    '--hidden_layers',
    nargs='+',
...
parser.add_argument(
    '--model_path',
    default='models/WNet1AdamLR000001EXTOnGen1Marg'
            'inNoConfidence240620_accuracy.pkl',
...
parser.add_argument(
    ...
    default=0.8,
    type=float,
    help='Split for testing the trained model on un-seen data'
    )
parser.add_argument(
    "--testing",
    default=False,
...
# Enabling mmap_mode uses very very less RAM for loading the
# array as it uses the array directly from the disk
image_embeds_orig = np.load(args.image_embeds, mmap_mode='r')
try:
    topk_embeds_orig = np.load(args.topk_embeds, mmap_mode='r')
except OSError:
    # This is for using the memory mapped array which can be
    # used to handle large arrays in numpy.
    topk_embeds_orig = np.memmap(
        args.topk_embeds,
        dtype=np.float32,
        mode='r',
        shape=(2109500, 2048)
...
def get_EmbedNet_embed(input_embedding):
    ...
    model.eval().double()
    loader = DataLoader(
        dataset=np.array(input_embedding),
        num_workers=1,
        batch_size=1024,
        shuffle=False,
        pin_memory=True
    ...
    for embedding in tqdm.tqdm(loader, desc='[INFO] NN pass'):
        if device.type == 'cpu':
...
if args.testing:
    ...
    image_test_count = int(image_embeds_orig.shape[0] - np.round(
        image_embeds_orig.shape[0] * args.test_split))
...
if args.cab and args.use_confidence:
    print("[INFO] Using the CAB module with alpha = {} "
          "and beta = {}...".format(args.cab_alpha, args.cab_beta))
...
for i in range(args.k):     # Looping over top k predictions
    ...
    for count in tqdm.tqdm(
            range(len(image_embeds)), desc='[INFO] K = {}'.format(i + 1)):
        total += 1
        # Getting the first embedding
        first_img_embed = image_embeds[count]
        # Getting top k embeddings corresponding to the first embedding
        corrs_topk_embeds = topk_embeds[topk_count: topk_count + i + 1]
        # Creating the KDTree for querying
        kdt = KDTree(corrs_topk_embeds, leaf_size=30, metric='euclidean')
        # Getting the distance and index by querying first embed on
        # corresponding text
        dist, ind = kdt.query(
            first_img_embed.reshape(1, -1),
            k=corrs_topk_embeds.shape[0],
            dualtree=True
            )
        ...
        if args.use_confidence:
            ...
            if args.cab:    # If selected, using the CAB module
                ...
                # Updated distace value after considering the confidence scores
                updatedDist = conf + dist[0]
            # Updated index value after considering the confidence scores
            newInd = ind[0][np.where(min(updatedDist) == updatedDist)[0][0]]
            # Updated predictions after considering the confidence scores
            pred = predictions[topk_count + newInd]
...
print('[INFO] Top {} accuracies are: {}.'.format(
    len(accuracyList),
    accuracyList)
    )
